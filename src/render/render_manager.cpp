...
void RenderManager::initialize(const mem::Allocator& allocator)
{
	...
	MaterialManager::initialize(allocator);
}
...
void RenderManager::shutdown()
{
	...
	for (RenderLayer& layer : data.order_table.layers)
	{
		for (RenderItemID item_id : layer.items.iter())
		{
			...
			{
				MaterialManager::material_destroy(item.material);
			}
		}
		layer.items.destroy();
	}
...
void RenderManager::recreate_window()
{
	Graphics::swap_chain_destroy(data.sc);
...
void RenderManager::update_viewport_transform(const Vector2I& viewport_size)
{
...
static inline void _update_batch_vb(RenderManager::Batch<BatchInstance>& batch, u32 stride)
{
...
template<typename BatchInstance>
static inline void _update_sprite_batch_vb(RenderManager::SpriteBatch<BatchInstance>& batch, u32 stride)
{
	if (batch.instance_count == 0)
		return;
...
void RenderManager::render_scene(Viewport* main_viewport)
{
	...
	{
		update_viewport_transform(main_viewport->viewport_size);
	...
	for (RenderLayer& layer : data.order_table.layers)
	{
		for (RenderItemID& item_id : layer.items.iter())
		{
			RenderItem& item = _get_item(item_id);
			_render_item(item);
		}
	}
    ...
    // TODO: Separate drawing and presenting.
...
void RenderManager::item_set_material(RenderItemID render_item_id, MaterialManager::MaterialID material)
{
	RenderItem& item = _get_item(render_item_id);
	item.material = material;
}

MaterialManager::MaterialID RenderManager::item_get_material(RenderItemID render_item_id)
{
	RenderItem& item = _get_item(render_item_id);
...
void RenderManager::render_item_draw_sprite(RenderItemID render_item_id, const Transform2D& transform,
	Graphics::TextureID texture, const Rect2D& rect, const Rect2D& src_rect, Color mod_color, RenderFlags flags)
{
	RenderItem& item = _get_item(render_item_id);
...
void RenderManager::render_item_draw_ui_sprite(RenderItemID render_item_id, const Transform2D& transform, Graphics::TextureID texture,
...
void RenderManager::_bind_material()
{
...
template<typename BatchInstance>
static inline void _end_prim_batch(Graphics::CommandBufferID cmd, RenderManager::Batch<BatchInstance>& batch, u32 stride)
{
	u32 offset = batch.buffer_offset;

	Graphics::command_buffer_bind_pipeline(cmd, batch.current_pipeline);
	Graphics::command_buffer_bind_index_buffer(cmd, RenderManager::data.global_ib, 0, Graphics::INDEX_TYPE_UINT8);
	...
	if constexpr (IsSame<BatchInstance, RenderManager::PrimitivePoint>)
	{
...
template<typename BatchInstance>
static inline void _end_sprite_batch(Graphics::CommandBufferID cmd, RenderManager::SpriteBatch<BatchInstance>& batch, u32 stride)
{
	u32 offset = batch.buffer_offset;
...
void RenderManager::_render_item(RenderItem& item)
{
	if (item.material != MaterialManager::MaterialID())
	{
...
void RenderManager::_update_scene_uniform()
{

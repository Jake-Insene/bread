...
void RenderManager::initialize(const mem::Allocator& allocator)
{
	...
	SceneRenderer::initialize(allocator);
}
...
{
	SceneRenderer::recreate_window();
}

void RenderManager::render_scene(Viewport* main_viewport)
{
	SceneRenderer::render_scene(main_viewport);
	reset_commands();
}

void RenderManager::present_scene()
{
	SceneRenderer::present_scene();
}
